...
# -------------------------------------------------------------------
# v1.1.8 Specialist Profile & Sections Fix
# -------------------------------------------------------------------
...
def ensure_user_hub_tables_v082_(db: Session):
    """
    v1.1.8 – bezpečné tabulky pro TIPy.
...
def api_tips_status_v082(db: Session = Depends(get_db)):
    try:
        ...
        return {"ok": False, "version": "1.1.8-partners-restore-visual-plus-grouping", "error": str(e)}




# -------------------------------------------------------------------
# v1.1.8 Adviser HUB routes fix
# -------------------------------------------------------------------
...
def hub_partners_v083(request: Request, q: str = "", selected: str = "", tab: str = "contacts", db: Session = Depends(get_db)):
    ...
    # v1.1.8 safe route fix: proměnné pro šablonu musí existovat vždy.
    try:
...
# -------------------------------------------------------------------
# v1.1.8 HUB Data Bridge – propojení uživatelského HUBu na admin data
# -------------------------------------------------------------------
...
def hub_partners_v084(
    ...
    """
    v1.1.8 – definitivní bezpečná uživatelská sekce Partneři.
...
def hub_contacts_v084(request: Request, q: str = "", db: Session = Depends(get_db)):
    # v1.1.8: hlavní menu Kontakty zobrazuje pouze globální kontakty ASTORIE.
...
# -------------------------------------------------------------------
# v1.1.8 TIP Admin Data Flow – sekce/podsekce/specialisté z adminu do poradce
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# v1.1.8 Partner autocomplete & Forms data source
# -------------------------------------------------------------------
...
def api_hub_partners_search_v086(q: str = "", limit: int = 20, db: Session = Depends(get_db)):
    ...
    if not table_exists_v084_(db, "partners"):
        return {"ok": True, "version": "1.1.8-partners-restore-visual-plus-grouping", "items": []}
...
def api_hub_partner_form_source_v086(partner_code: str, db: Session = Depends(get_db)):
    ...
    if not table_exists_v084_(db, "partners"):
        return {"ok": False, "version": "1.1.8-partners-restore-visual-plus-grouping", "error": "Tabulka partners neexistuje."}
    ...
    if not partner:
        return {"ok": False, "version": "1.1.8-partners-restore-visual-plus-grouping", "error": "Partner nenalezen."}
...
# -------------------------------------------------------------------
# v1.1.8 Operational TIP Workflow
...
# -------------------------------------------------------------------
# v1.1.8 Unified TIP Inbox – jedna obrazovka jako ve stávající aplikaci
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# v1.1.8 XLSX importer – import přímo ze staženého Google Sheetu
# -------------------------------------------------------------------
...
def xlsx_upsert_v093_(db, table, conflict_col, data, update_existing=False):
    ...
    v1.1.8: u UUID tabulek doplňuje id ručně, protože starší PostgreSQL tabulky
    ...
    # v1.1.8: produkční tabulky mohou mít NOT NULL created_at/updated_at bez DB defaultu.
    ...
    # v1.1.8: tabulka subsections má v produkci povinný section_id.
...
def import_hub_xlsx_data_v093_(db, wb, update_existing=False):
    ...
    # Specialisté
    # v1.1.8: index se nevytváří uvnitř importu. Připravuje se bezpečně před importem.
...
# -------------------------------------------------------------------
# v1.1.8 import hardening endpoints
...
async def api_import_hub_xlsx_upload_v094(
    ...
    try:
        ...
        result["ok"] = len(result.get("errors", [])) == 0
        result["mode"] = "update_existing" if update_existing == "1" else "safe_insert_only"
        result["version"] = "1.1.8-partners-restore-visual-plus-grouping"
        return result
    except Exception as exc:
        return {
            "ok": False,
...
# -------------------------------------------------------------------
# v1.1.8 import schema canonical fix
...
# -------------------------------------------------------------------
# v1.1.8 import relationship fix
...
# -------------------------------------------------------------------
# v1.1.8 uuid relationship fix endpoint
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# v1.1.8 import cleanup + partner UI completion
# Cíl:
...
# -------------------------------------------------------------------
# v1.1.8 Partner Workflow Core
# -------------------------------------------------------------------
PARTNER_WORKFLOW_VERSION = "1.1.8-partners-restore-visual-plus-grouping"
...
# -------------------------------------------------------------------
# v1.1.8 Partner Workflow UX Upgrade
...
PARTNER_WORKFLOW_UX_VERSION = "1.1.8-partners-restore-visual-plus-grouping"
...
def ensure_partner_ux_v111(db: Session):
    ...
    # Doplnit drobné tabulky, pokud v1.1.8 nebyla nasazená.
...
# -------------------------------------------------------------------
# v1.1.8 Partner hotfix safe UI
# /hub/partners uses safe template. No import/data destructive changes.
# -------------------------------------------------------------------
PARTNER_HOTFIX_VERSION = "1.1.8-partners-restore-visual-plus-grouping"
...
def api_partners_restore_visual_status_v118(db: Session = Depends(get_db)):
    """
    v1.1.8 – kontrola opravné verze sekce Partneři.
