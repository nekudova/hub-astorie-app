...
# -------------------------------------------------------------------
# v1.2.0 Specialist Profile & Sections Fix
# -------------------------------------------------------------------
...
def api_visible_sections_v072(db: Session = Depends(get_db)):
    ...
    return {
        "ok": True,
        "version": "1.2.0-safe-rollback-visual-shell",
...
def ensure_user_hub_tables_v082_(db: Session):
    """
    v1.2.0 – bezpečné tabulky pro TIPy.
...
def api_tips_status_v082(db: Session = Depends(get_db)):
    try:
        ...
        return {"ok": False, "version": "1.2.0-safe-rollback-visual-shell", "error": str(e)}




# -------------------------------------------------------------------
# v1.2.0 Adviser HUB routes fix
# -------------------------------------------------------------------
...
def hub_partners_v083(request: Request, q: str = "", selected: str = "", tab: str = "contacts", db: Session = Depends(get_db)):
    ...
    # v1.2.0 safe route fix: proměnné pro šablonu musí existovat vždy.
    try:
...
# -------------------------------------------------------------------
# v1.2.0 HUB Data Bridge – propojení uživatelského HUBu na admin data
# -------------------------------------------------------------------
...
def hub_partners_v084(
    ...
    """
    v1.2.0 – definitivní bezpečná uživatelská sekce Partneři.
...
def hub_contacts_v084(request: Request, q: str = "", db: Session = Depends(get_db)):
    # v1.2.0: hlavní menu Kontakty zobrazuje pouze globální kontakty ASTORIE.
...
# -------------------------------------------------------------------
# v1.2.0 TIP Admin Data Flow – sekce/podsekce/specialisté z adminu do poradce
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# v1.2.0 Partner autocomplete & Forms data source
# -------------------------------------------------------------------
...
def api_hub_partners_search_v086(q: str = "", limit: int = 20, db: Session = Depends(get_db)):
    ...
    if not table_exists_v084_(db, "partners"):
        return {"ok": True, "version": "1.2.0-safe-rollback-visual-shell", "items": []}
...
def api_hub_partner_form_source_v086(partner_code: str, db: Session = Depends(get_db)):
    ...
    if not table_exists_v084_(db, "partners"):
        return {"ok": False, "version": "1.2.0-safe-rollback-visual-shell", "error": "Tabulka partners neexistuje."}
    ...
    if not partner:
        return {"ok": False, "version": "1.2.0-safe-rollback-visual-shell", "error": "Partner nenalezen."}
...
# -------------------------------------------------------------------
# v1.2.0 Operational TIP Workflow
...
# -------------------------------------------------------------------
# v1.2.0 Unified TIP Inbox – jedna obrazovka jako ve stávající aplikaci
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# v1.2.0 XLSX importer – import přímo ze staženého Google Sheetu
# -------------------------------------------------------------------
...
def xlsx_upsert_v093_(db, table, conflict_col, data, update_existing=False):
    ...
    v1.2.0: u UUID tabulek doplňuje id ručně, protože starší PostgreSQL tabulky
    ...
    # v1.2.0: produkční tabulky mohou mít NOT NULL created_at/updated_at bez DB defaultu.
    ...
    # v1.2.0: tabulka subsections má v produkci povinný section_id.
...
def import_hub_xlsx_data_v093_(db, wb, update_existing=False):
    ...
    # Specialisté
    # v1.2.0: index se nevytváří uvnitř importu. Připravuje se bezpečně před importem.
...
# -------------------------------------------------------------------
# v1.2.0 import hardening endpoints
...
async def api_import_hub_xlsx_upload_v094(
    ...
    try:
        ...
        result["ok"] = len(result.get("errors", [])) == 0
        result["mode"] = "update_existing" if update_existing == "1" else "safe_insert_only"
        result["version"] = "1.2.0-safe-rollback-visual-shell"
        return result
    except Exception as exc:
        return {
            "ok": False,
...
# -------------------------------------------------------------------
# v1.2.0 import schema canonical fix
...
# -------------------------------------------------------------------
# v1.2.0 import relationship fix
...
# -------------------------------------------------------------------
# v1.2.0 uuid relationship fix endpoint
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# v1.2.0 import cleanup + partner UI completion
# Cíl:
...
# -------------------------------------------------------------------
# v1.2.0 Partner Workflow Core
# -------------------------------------------------------------------
PARTNER_WORKFLOW_VERSION = "1.2.0-safe-rollback-visual-shell"
...
# -------------------------------------------------------------------
# v1.2.0 Partner Workflow UX Upgrade
...
PARTNER_WORKFLOW_UX_VERSION = "1.2.0-safe-rollback-visual-shell"
...
def ensure_partner_ux_v111(db: Session):
    ...
    # Doplnit drobné tabulky, pokud v1.2.0 nebyla nasazená.
...
# -------------------------------------------------------------------
# v1.2.0 Partner hotfix safe UI
# /hub/partners uses safe template. No import/data destructive changes.
# -------------------------------------------------------------------
PARTNER_HOTFIX_VERSION = "1.2.0-safe-rollback-visual-shell"
...
def api_partners_restore_visual_status_v118(db: Session = Depends(get_db)):
    """
    v1.2.0 – kontrola opravné verze sekce Partneři.
...
def api_safe_rollback_visual_shell_status_v120(db: Session = Depends(get_db)):
    """
    v1.2.0 – bezpečný rollback destruktivních UI zásahů.
    ...
    return {
        "ok": len(errors) == 0,
        "version": "1.2.0-safe-rollback-visual-shell",
        "message": "Stabilní rollback na funkční šablony v1.1.8 + neinvazivní sjednocení vizuálu. DB/import/workflow beze změny.",
        "tables": tables,
        "errors": errors,
    }
